...
import numpy as np
from networks.gan.ganf.NF import MAF, RealNVP
    ...
    def predict_prob(self, test_iterator, window_labels=None):
        ...
        with torch.no_grad():
            for x in test_iterator:
                ...
                x = x.to(device)
                loss = -self.test(x, A.df).cpu().numpy()
    ...
    def fit(
        ...
        for _ in range(epochs):
            ...
            for x in train_loader:
                ...
                A.df.copy_(torch.clamp(A.df, min=0, max=1))
            logging.info(
            ...
            if val_loader is not None:
                ...
                if np.mean(loss_val) < loss_best:
                    ...
                    torch.save(A.df, os.path.join(self.model_root, "graph.pt"))
                    torch.save(
                        self.state_dict(),
