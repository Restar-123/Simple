...
class BatchNorm(nn.Module):
    ...
    def forward(self, x, cond_y=None):
        if self.training:
            ...
            # update running mean
            self.running_mean.mul_(self.momentum).add_(self.batch_mean.df * (1 - self.momentum))
            self.running_var.mul_(self.momentum).add_(self.batch_var.df * (1 - self.momentum))
