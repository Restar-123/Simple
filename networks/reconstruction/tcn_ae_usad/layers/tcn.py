...
class TCN(torch.nn.Module):
    ...
    def __init__(self,
                 input_dim: int,
                 nb_filters: Union[int, Sequence[int]] = 20,
                 kernel_size: int = 20,
                 nb_stacks: int = 1,
...
if __name__ == '__main__':
    model = TCNResidualBlock(input_dim=3,dilation_rate=1,nb_filters=20,kernel_size=3,padding="same")
    input = torch.randn(64,3,50)
    input = input
    output = model(input)
    print(output)