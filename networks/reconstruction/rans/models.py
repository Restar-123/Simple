...
class sincoder(Layer):
    ...
    def build(self, input_shape):
        ...
        if self.freq_init and not self.trainable_freq:
            self.freq = [self.add_weight(initializer=tf.keras.initializers.Constant(f), trainable=False) for f in self.freq_init]
        elif self.freq_init:
            self.freq = [self.add_weight(initializer=tf.keras.initializers.Constant(f), constraint=NonNeg(), trainable=True) for f in self.freq_init]
        else:
            self.freq = [
