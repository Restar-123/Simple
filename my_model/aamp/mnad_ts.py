...
if __name__ == '__main__':
    ...
    # 创建 Memory 模块
    memory_module = Memory(10, 8, 8, temp_update=0.1, temp_gather=0.1)

    # 创建查询数据和记忆数据
    query = torch.randn(12, 8, 32, 32).cuda()  # 查询数据
    keys = torch.randn(10, 8).cuda()  # 记忆数据

    # 运行 Memory 模块
    updated_query, updated_memory, softmax_score_query, softmax_score_memory, gathering_loss, spreading_loss = memory_module(
        query, keys, train=True)

    # 输出结果
    print("Updated Query Shape:", updated_query.shape)
    print("Updated Memory Shape:", updated_memory.shape)
    print("Softmax Score Query Shape:", softmax_score_query.shape)
    print("Softmax Score Memory Shape:", softmax_score_memory.shape)
    print("Gathering Loss:", gathering_loss.shape)
    print("Spreading Loss:", spreading_loss.shape)
